...
try:
    ...
    data['date'] = pd.to_datetime(data['date'], errors='coerce')  # Konversi kolom tanggal
    data = data[data['date'].dt.year == 2022]  # Filter hanya untuk tahun 2022

    if data.empty:
        st.warning("Dataset tidak memiliki data untuk tahun 2022.")
    else:
        # Tampilkan dataset setelah filter
        st.subheader("Dataset Tahun 2022")
        st.write(f"Dataset berhasil difilter untuk tahun 2022. Jumlah data: {len(data)}")
        st.dataframe(data)
